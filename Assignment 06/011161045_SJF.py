...
class SSQ:
    ...
    def arrival(self):
        ...
        if self.server_status == 0:
            ...
            z_lists = zip(self.service_times_in_queue, self.times_of_arrival_in_queue)
            s_z_lists = sorted(z_lists)
            self.times_of_arrival_in_queue = [element for _, element in s_z_lists]

            self.service_times_in_queue.sort()
